...
//       WebDriver driver = new FirefoxDriver();
       ...
       String genXpath = "//label[text()='" + strGender + "']/following-sibling::input";
		...
		gen.click();
List<WebElement> Months = driver.findElements(By.xpath("//select[@title='Month']/option"));//all months
        
        for(WebElement elm : Months) {
            
            System.out.println(elm.getText());
            
            WebElement CurrMonth = driver.findElement(By.xpath("//select[@title='Month']/option[@selected='1']")); //to find perticular month 
            
            System.out.println("Current Month " + CurrMonth.getText());
            
WebElement Day = driver.findElement(By.xpath("//select[@id='day']"));
            
            Select ddDay = new Select(Day);        //to select exact date
             ddDay.selectByVisibleText("25");
             
 WebElement Month = driver.findElement(By.xpath("//select[@id='month']"));
            
   Select ddMonth = new Select(Month);        //to select exact date
      ddMonth.selectByVisibleText("Aug");
             
      WebElement Year = driver.findElement(By.xpath("//select[@id='year']"));
      
      Select ddYear= new Select(Year);        //to select exact date
